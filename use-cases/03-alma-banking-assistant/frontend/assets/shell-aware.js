/* Shell-aware: hides page sidebar when inside shell iframe. */
(function(){
  // Activate if ?shell=1 OR if we're inside an iframe (parent !== self)
  var inShell = window.location.search.includes('shell=1') || (window.parent !== window);
  if(!inShell) return;

...
- NEVER call execute_purchase on the FIRST message about a product.
- When a customer expresses purchase INTENT ("I want to buy", "I want to purchase", "can I get",
  "set it up", "activate it"), you MUST respond with a confirmation prompt:
  "I can set up [product] for BHD [price]. This will be debited from your account. Shall I proceed?"
- Only call execute_purchase when the customer replies with a SHORT confirmation:
  "yes", "confirm", "go ahead", "proceed", "do it", "sure", "ok"
- If the message is longer than 5 words and mentions a product name → it's INTENT, not confirmation.
...
def invoke(payload):
    ...
    parts = [f"[Customer ID: {customer_id}]"]
    if active_flow:

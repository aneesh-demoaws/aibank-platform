...
def load_segment_configuration(customer_segment):
    """Load segment-specific configuration from DynamoDB with fallback to in-code defaults.

    Fallback chain:
      1. aibank-loan-config table (config_type='segment', config_id=<segment>)  — business-editable source of truth
      2. get_default_configuration(segment)  — in-code baseline + get_segment_specific_adjustments()
      3. get_emergency_fallback_configuration()  — conservative values if anything explodes
    """
    ...
    try:
        ...
        if 'Item' in resp:
            ...
            return config
        logger.warning(
            f"No row in aibank-loan-config for segment '{customer_segment}' — "
            f"falling back to in-code default configuration. Seed the DynamoDB row to silence this warning."
        )
    except Exception as e:
        logger.error(f"DynamoDB segment config lookup failed for '{customer_segment}': {e} — using in-code defaults")

    try:
        config = get_default_configuration(customer_segment)
        config['source'] = config.get('source', 'default')
        logger.info(f"Using in-code default configuration for segment: {customer_segment}")
        return config
    except Exception as e:
        logger.error(f"Default configuration build failed for '{customer_segment}': {e} — using emergency fallback")

...
ALLOWED_ORIGINS = ["https://aibank.demoaws.com", "https://d1pfo41ge1bxh5.cloudfront.net"]
...
def _rows(resp):
    cols = [c.get("label") or c["name"] for c in resp["columnMetadata"]]
    rows = []
...
def _cors(status, body, event=None):
    origin = "*"  # Internal employee tool — allow all origins
    return {
        "statusCode": status,
        "headers": {
